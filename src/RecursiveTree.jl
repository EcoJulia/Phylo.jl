...
"""
Base.@kwdef mutable struct RecursiveElt{RT, NL, MyType <: AbstractElt{RT, NL},
                                        MyData,
                                        TheirType <: AbstractElt{RT, NL},
                                        TheirData,
                                        BT <: BranchingType,
                                        LenUnits <: Number} <:
                           AbstractElt{RT, NL}
    ...
    BT, LenUnits}} = RecursiveElt{RT, NL, TheirType, TheirData, MyType, MyData,
                                  BT, LenUnits}[]
    data::MyData = _emptydata(MyData)
    length::Union{LenUnits, Missing} = missing
end

const RecursiveNode{RT, NL, NodeData, BranchData, BT, LenUnits} = RecursiveElt{RT,
                                                                               NL,
                                                                               AbstractNode{RT,
                                                                                            NL},
                                                                               NodeData,
...
const RecursiveBranch{RT, NL, NodeData, BranchData, BT, LenUnits} = RecursiveElt{RT,
                                                                                 NL,
                                                                                 AbstractBranch{RT,
                                                                                                NL},
                                                                                 BranchData,
...
function RecursiveNode{RT, NL, NodeData, BranchData, BT, LenUnits}(name::NL,
                                                                   data::NodeData = nothing) where
         {RT, NL, NodeData, BranchData, BT, LenUnits}
    return RecursiveNode{RT, NL, NodeData, BranchData,
...
"""
mutable struct RecursiveTree{RT, NL, NodeData, BranchData, BT <: BranchingType,
                             LenUnits, TD} <:
               AbstractTree{OneTree, RT, NL,
                            RecursiveNode{RT, NL, NodeData, BranchData, BT,
                                          LenUnits},
                            RecursiveBranch{RT, NL, NodeData, BranchData, BT,
                                            LenUnits}}
    name::String
    nodedict::Dict{NL, Int}
    roots::Vector{RecursiveNode{RT, NL, NodeData, BranchData, BT, LenUnits}}
    nodes::Vector{Union{RecursiveNode{RT, NL, NodeData, BranchData, BT,
                                      LenUnits},
                        Missing}}
    branches::Vector{Union{RecursiveBranch{RT, NL, NodeData, BranchData, BT,
                                           LenUnits},
                           Missing}}
    ...
    cache::Dict{TraversalOrder,
                Vector{RecursiveNode{RT, NL, NodeData, BranchData, BT,
                                     LenUnits}}}
...
function RecursiveTree{RT, NL, NodeData, BranchData, BT, LenUnits, TD}(leafinfos::TD) where
    ...
    return RecursiveTree{RT, NL, NodeData, BranchData, BT, LenUnits, TD}(leafnames;
                                                                         tipdata = leafinfos)
end
            ...
            if !ismissing(node)
                if i ≠ node.id || i ≠ tree.nodedict[node.name]
                    ...
                    if RT <: Rooted
                        ...
                        if BT ≡ BinaryBranching &&
                           length(getoutbounds(tree, node)) > 2
                            tree.isvalid = false
                            @warn "Node name $(node.name) has too many outbound connections"
                        end
                    else # Unrooted
                        if BT ≡ BinaryBranching &&
                           length(getoutbounds(tree, node)) > 3
                            tree.isvalid = false
                            @warn "Node name $(node.name) has too many outbound connections"
                        end
                    end
...
function _invalidate!(tree::RecursiveTree, state = missing)
    empty!(tree.cache)
    return tree.isvalid = state
end

# Type aliases

const ReB{RT, BT, LenUnits} = RecursiveBranch{RT, String, Dict{String, Any},
                                              Dict{String, Any}, BT, LenUnits}
const ReN{RT, BT, LenUnits} = RecursiveNode{RT, String, Dict{String, Any},
                                            Dict{String, Any}, BT, LenUnits}
const ReT{RT, TD, BT, LenUnits} = RecursiveTree{RT, String, Dict{String, Any},
                                                Dict{String, Any}, BT, LenUnits,
                                                TD}
...
function _emptydata(::Type{RecursiveNode{RT, NL, NodeData, BranchData}}) where
         {RT, NL, NodeData, BranchData}
    return _newdata(NodeData)
end
function _emptydata(::Type{RecursiveBranch{RT, NL, NodeData, BranchData}}) where
         {RT, NL, NodeData, BranchData}
    return _newdata(BranchData)
end
...
function _gettdlabels(t::RecursiveTree{RT, NL, NodeData, BranchData,
                                       BT, LenUnits, TD}) where
         {RT, NL, NodeData, BranchData, BT, LenUnits, TD}
    return collect(keys(getleafinfo(t)))
end
function _gettdlabels(t::RecursiveTree{RT, NL, NodeData, BranchData,
                                       BT, LenUnits, DataFrame}) where
         {RT, NL, NodeData, BranchData, BT, LenUnits}
    return unique(getleafinfo(t)[:, 1])
end
...
function _leafinfotype(::Type{<:RecursiveTree{RT, NL, NodeData, BranchData, BT,
                                              LenUnits, TD}}) where
         {RT, NL, NodeData, BranchData, BT, LenUnits, TD}
    return TD
end
...
function _setleafinfo!(tree::RecursiveTree{RT, NL, NodeData, BranchData, BT,
                                           LenUnits, TD},
                       leafinfo::TD) where
         {RT, NL, NodeData, BranchData, BT, LenUnits, TD}
    return (tree.tipdata = leafinfo)
end
...
function _hasnode(tree::RecursiveTree{RT, NL}, name::NL) where {RT, NL}
    return haskey(tree.nodedict, name)
end
function _hasnode(tree::RecursiveTree{RT, NL},
                  node::N) where
         {RT, NL, N <: RecursiveNode{RT, NL}}
    return haskey(tree.nodedict, node.name)
end

import Phylo.API: _getnode
_getnode(::RecursiveTree, node::RecursiveNode) = node
function _getnode(tree::RecursiveTree{RT, NL}, name::NL) where {RT, NL}
    return tree.nodes[tree.nodedict[name]]
end
...
function _createnode!(tree::RecursiveTree{RT, NL, NodeData, BranchData, BT,
                                          LenUnits, TD},
                      name::Union{NL, Missing},
                      data::NodeData = _emptydata(NodeData)) where
...
function _createnode!(tree::RecursiveTree{Unrooted, NL, NodeData, BranchData,
                                          BT, LenUnits, TD},
                      name::Union{NL, Missing},
                      data::NodeData = _emptydata(NodeData)) where
...
function _hasinbound(::RecursiveTree, node::RecursiveNode{<:Rooted})
    return !isnothing(node.in)
end
...
function _getconnections(::RecursiveTree, node::RecursiveNode{Unrooted},
                         exclude)
    return filter(∉(exclude), node.conns)
end
...
function _addinbound!(tree::RecursiveTree{RT},
    ...
    node.in = branch
    filter!(n -> n ≠ node, tree.roots)
    return _invalidate!(tree)
end

import Phylo.API: _removeinbound!
function _removeinbound!(tree::RecursiveTree{RT},
                         node::RecursiveNode{RT},
                         branch::RecursiveBranch{RT} = node.in) where {
                                                                       RT <:
                                                                       Rooted}
    _hasinbound(tree, node) ||
        error("Node $(node.name) has no inbound connection")
    node.in ≡ branch ||
...
function _hasoutboundspace(::RecursiveTree{<:Rooted, NL, TheirData, MyData,
                                           BinaryBranching, LenUnits, TD},
                           node::RecursiveNode{<:Rooted, NL, TheirData, MyData,
                                               BinaryBranching, LenUnits}) where
         {NL, TheirData, MyData, TD, LenUnits}
    return length(node.conns) < 2
end

import Phylo.API: _hasspace
function _hasspace(::RecursiveTree{Unrooted, NL, TheirData, MyData,
                                   BinaryBranching, LenUnits, TD},
                   node::RecursiveNode{Unrooted, NL, TheirData, MyData,
                                       BinaryBranching, LenUnits}) where
         {NL, TheirData, MyData, LenUnits, TD}
    return length(node.conns) < 3
end
...
function _getbranchnames(tree::RecursiveTree)
    return [b.id for b in skipmissing(tree.branches)]
end

import Phylo.API: _hasbranch
function _hasbranch(tree::RecursiveTree, id::Int)
    return 1 ≤ id ≤ length(tree.branches) && !ismissing(tree.branches[id])
end
function _hasbranch(tree::RecursiveTree, branch::RecursiveBranch)
    return branch ≡ tree.branches[branch.id]
end
...
function _branchdims(::Type{<:RecursiveBranch{RT, NL, NodeData, BranchData, BT,
                                              LenUnits}}) where
         {RT, NL, NodeData, BranchData, BT, LenUnits}
    return dimension(LenUnits)
end
...
function _conn(::RecursiveTree{Unrooted}, branch::RecursiveBranch{Unrooted},
               exclude::RecursiveNode{Unrooted})
    return exclude ≡ branch.conns[1] ? branch.conns[2] :
           (exclude ≡ branch.conns[2] ? branch.conns[1] :
            error("Branch $(branch.id) not connected to node $(exclude.name)"))
end

import Phylo.API: _conns
function _conns(::RecursiveTree{Unrooted}, branch::RecursiveBranch{Unrooted})
    return branch.conns
end
...
function _nodedatatype(::Type{<:RecursiveTree{RT, NL, NodeData, BranchData}}) where
         {RT, NL, NodeData, BranchData}
    return NodeData
end
function _nodedatatype(::Type{<:RecursiveNode{RT, NL, NodeData, BranchData}}) where
         {RT, NL, NodeData, BranchData}
    return NodeData
end
...
function _branchdatatype(::Type{<:RecursiveTree{RT, NL, NodeData, BranchData}}) where
         {RT, NL, NodeData, BranchData}
    return BranchData
end
function _branchdatatype(::Type{<:RecursiveBranch{RT, NL, NodeData, BranchData}}) where
         {RT, NL, NodeData, BranchData}
    return BranchData
end
...
function _setbranchdata!(::RecursiveTree{RT, NL, NodeData, BranchData},
...
function _createbranch!(tree::RecursiveTree{RT, NL, NodeData, BranchData, BT,
                                            LenUnits, TD},
                        from::RecursiveNode{RT, NL, NodeData, BranchData, BT,
                                            LenUnits},
                        to::RecursiveNode{RT, NL, NodeData, BranchData, BT,
                                          LenUnits},
...
function _createbranch!(tree::RecursiveTree{Unrooted, NL, NodeData, BranchData,
                                            BT, LenUnits, TD},
                        from::RecursiveNode{Unrooted, NL, NodeData, BranchData,
                                            BT, LenUnits},
                        to::RecursiveNode{Unrooted, NL, NodeData, BranchData,
                                          BT, LenUnits},
                        len::Union{LenUnits, Missing} = missing,
                        name = missing,
                        data::BranchData = _emptydata(BranchData)) where
...
function _deletebranch!(tree::RecursiveTree{RT},
                        branch::RecursiveBranch{RT}) where {RT <: Rooted}
...
function _deletebranch!(tree::RecursiveTree{Unrooted},
                        branch::RecursiveBranch{Unrooted})
...
import Base.show
function show(io::IO, ::MIME"text/plain", node::RecursiveNode{Unrooted})
    return length(node.conns) == 0 ?
...
function show(io::IO, node::RecursiveNode{Unrooted})
    ...
    elseif nc == 1
        ...
        print(io,
              "with $nc outbound connections (branches $(getfield.(node.conns, :id)))")
    end
end

function show(io::IO, ::MIME"text/plain", node::RecursiveNode{<:Rooted})
    return isnothing(node.in) ?
           (length(node.conns) == 0 ?
            print(io, "unattached node '$(node.name)'") :
...
function show(io::IO, node::RecursiveNode{RT}) where {RT <: Rooted}
    ...
    if isnothing(node.in)
        ...
        elseif no == 1
            print(io,
                  "a root node with 1 outbound connection" *
                  " (branch $(node.conns[1].id))")
        else
            print(io,
                  "a root node with $no outbound connections" *
                  " (branches $(getfield.(node.conns, :id)))")
        end
    else
        if no == 0
            print(io,
                  "a leaf with an incoming connection (branch $(node.in.id))")
        elseif no == 1
            print(io,
                  "an internal node with 1 inbound and 1 outbound connection " *
                  "(branches $(node.in.id) and $(node.conns[1].id))")
        else
            print(io,
                  "an internal node with 1 inbound and $no outbound connections" *
                  " (branches $(node.in.id) and $(getfield.(node.conns, :id)))")
        end
    end
end

function show(io::IO, ::MIME"text/plain", branch::RecursiveBranch)
    return print(io, "branch '$(branch.id)'")
end

function show(io::IO, branch::RecursiveBranch{Unrooted})
    return print(io,
                 "RecursiveBranch{Unrooted} $(branch.id), connecting nodes" *
...
function show(io::IO, branch::RecursiveBranch{RT}) where {RT <: Rooted}
    return print(io,
                 "RecursiveBranch{$RT} $(branch.id), from node '$(branch.in.name)'" *
...
function _renametipdata!(tree::RecursiveTree{RT, NL, ND, BD, BT, LU, Nothing},
